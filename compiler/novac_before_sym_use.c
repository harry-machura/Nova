...
static void parse_logical_and(P* p) {
    parse_cmp(p);
    while (match(&p->L, T_ANDAND)) {
        emit_op(p->out, OP_JZ);
        size_t jz_off = p->out->len;
        emiti32(p->out, 0);
        parse_cmp(p);
        emit_op(p->out, OP_JMP);
        size_t jmp_over_false = p->out->len;
        emiti32(p->out, 0);
        size_t false_pc = p->out->len;
        emit_op(p->out, OP_PUSHI);
        emiti32(p->out, 0);
        size_t end2_pc = p->out->len;
        patch_i32_at(p->out, jz_off, (int32_t)false_pc);
        patch_i32_at(p->out, jmp_over_false, (int32_t)end2_pc);
    }
}

static void parse_logical_or(P* p) {
    parse_logical_and(p);
    while (match(&p->L, T_OROR)) {
        emit_op(p->out, OP_JZ);
        size_t jz_to_rhs = p->out->len;
        emiti32(p->out, 0);
        emit_op(p->out, OP_PUSHI);
        emiti32(p->out, 1);
        emit_op(p->out, OP_JMP);
        size_t jmp_over_rhs = p->out->len;
        emiti32(p->out, 0);
        size_t rhs_pc = p->out->len;
        parse_logical_and(p);
        size_t end_pc = p->out->len;
        patch_i32_at(p->out, jz_to_rhs, (int32_t)rhs_pc);
        patch_i32_at(p->out, jmp_over_rhs, (int32_t)end_pc);
    }
}